...
{
    public partial class Smartphones_Apple_Add : MaterialForm
    {
        ...
        private void Add_Click(object sender, EventArgs e)
        {
            ...
            myCommand.CommandText = $"insert into smartphones_apple(model,class,price,cpu,display,dram,memory,battery,color,quantity,consignment,receipt_date) values('{textBox1.Text}','{textBox3.Text}',{textBox4.Text},'{textBox5.Text}','{textBox6.Text}',{textBox7.Text},{textBox8.Text},{textBox9.Text},'{textBox10.Text}',{textBox11.Text},'{textBox12.Text}','{dateTimePicker1.Value.Date.ToString("yyyy-MM-dd")}')";

            myCommand.ExecuteNonQuery();

            myCommand.CommandText = $"select count(*) from smartphones_apple_warehouse where model = '{model}' and class = '{class1}' and price = {price} and cpu = '{cpu}' and display = '{display}' and dram = {dram} and memory = {memory} and battery = {battery} and color = '{color}' and quantity = {quantity}";

            int cnt = Convert.ToInt32(myCommand.ExecuteScalar());

            if (cnt != 0)
                myCommand.CommandText = $"update smartphones_apple_warehouse set quantity = quantity + {quantity} where model = '{model}' and class = '{class1}' and price = {price} and cpu = '{cpu}' and display = '{display}' and dram = {dram} and memory = {memory} and battery = {battery} and color = '{color}'";
            else
                myCommand.CommandText = $"insert into smartphones_apple_warehouse(model,class,price,cpu,display,dram,memory,battery,color,quantity) values('{textBox1.Text}','{textBox3.Text}',{textBox4.Text},'{textBox5.Text}','{textBox6.Text}',{textBox7.Text},{textBox8.Text},{textBox9.Text},'{textBox10.Text}',{textBox11.Text})";
        ...
        private void Smartphones_Apple_Add_Load(object sender, EventArgs e)
        {
            textBox1.Text = "";
            textBox3.Text = "";
            textBox4.Text = "";
